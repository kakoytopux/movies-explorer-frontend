...
export default function Navigation() {
  ...
  function setMenu(evt) {
    if(evt.target.classList.contains('setBurgerMenu')) {
      setBurgerMenu(!burgerMenu);
    }
  }

  return (
    <nav className="navigation setBurgerMenu" onClick={setMenu}>
      <div className={`navigation__container ${burgerMenu ? 'navigation__container_active' : ''}`}>
        <ul className='navigation__lists lists'>
          ...
          <li className="navigation__list navigation__list_type_profile navigation__list_type_display">
            <Link to='/profile' className={`link-profile link ${burgerMenu ? 'link-profile_visible' : ''}`}>Аккаунт</Link>
          </li>
        </ul>
        <div className="navigation__cross setBurgerMenu" onClick={setMenu}></div>
      </div>
    </nav>
  );
}
